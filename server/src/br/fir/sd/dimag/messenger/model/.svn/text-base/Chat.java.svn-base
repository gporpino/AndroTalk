...
public class Chat implements Serializable{
	...
	public List<ChatMessage> getNotSentToDestinyMessages(){
		ArrayList<ChatMessage> list = new ArrayList<ChatMessage>();
		for (ChatMessage message : messages){
			if (message.getMessageStauts() != MessageStatus.SENT_TO_DESTINY){
				list.add(message);
			}
		}
		return list;
	...
	public List<ChatMessage> getNotSentToUserMessages( User user){
		ArrayList<ChatMessage> list = new ArrayList<ChatMessage>();
		for (ChatMessage message : messages){
			
			if (message.getMessageStauts() != MessageStatus.SENT_TO_DESTINY && 
					!user.getLogin().equals(message.getUserLoginFrom())){
				list.add(message);
			}
		}
		return list;
